            ...
            product.map((item) => (
              <tbody>
                <tr>
                  ...
                  <td>{item.quantity}</td>
                  <td>{item.category}</td>
                  <td ><Button style={{ backgroundColor: 'gray', border: 'none' }} className='m-2' onClick={() => Nvgt(`/edit/${item._id}`)}>Edit</Button>
                    <Button style={{ backgroundColor: 'red', border: 'none' }} onClick={() => removeItem(item._id)}>Delete</Button></td>
                </tr>
              </tbody>
            ))
          }
